...
The module is **vendored**, not shared: ``backend/diag.py``. Every repo in the fleet
...
| Platform | Directory |
| --- | --- |
| macOS | `~/Library/Logs/pmse-to-wwb/` |
| Linux | `$XDG_STATE_HOME/pmse-to-wwb/logs/` (default `~/.local/state/...`) |
| Windows | `%LOCALAPPDATA%\pmse-to-wwb\logs\` |

`PMSE_TO_WWB_LOG_DIR` overrides it. The path is printed on the first line of every
...
`pmse-to-wwb.YYYY-MM-DD.log`, rotated daily, seven kept, no colour escapes.

Verbosity comes from `PMSE_TO_WWB_LOG`, default `INFO`.
...
Writes `pmse-to-wwb-diagnostics-<timestamp>.json` and prints its path. One file, so
